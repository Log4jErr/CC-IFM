...
'use strict';
    ...
    containerDefineHint: '拖入 {name}：请填写容器名称（角色已设为{role}），保存后生效',
            ...
            missingMachineRemove: '从机器里移除这个外设引用（机器不再引用它）',
            missingMachineRemoveConfirm: '确定让机器不再引用「{name}」吗？（只移除机器里的这条引用）',
    ...
    containerDefineHint: 'Dropped {name}: give the container a name (role is set to {role}) and save',
            ...
            missingMachineRemove: 'Remove this peripheral reference from the machine',
            missingMachineRemoveConfirm: 'Make the machine stop referencing "{name}"? (only that reference is removed)',
